...
**New: Error Utilities (`convex/lib/errors.ts`):**
- Added `assertExists()` helper function for validating entities before database operations
- Provides consistent error messages with entity type and ID

**Fixed: Silent Mutation Failures (`convex/generatedVideos.ts`):**
- `incrementViewCount`: Now throws error instead of silently returning when video not found
- `incrementSentCount`: Now throws error instead of silently returning when video not found
- `updateHeygenId`: Added entity validation before patch operation
- `updateAfterGeneration`: Added entity validation before patch operation
- `togglePublic`: Added entity validation before patch operation

**Fixed: Batch Operation Error Handling (`convex/feed.ts`):**
- `generateFeed` mutation now wraps feed item insertions in try-catch
- Errors are logged and re-thrown for proper error propagation

**Fixed: HeyGen Webhook Convex Integration (`app/api/heygen/webhook/route.ts`):**
- Completed previously commented-out Convex integration
- Webhook now properly updates `videoGenerationJobs` status on completion/failure
- Webhook now properly updates `generatedVideos` with video URL, thumbnail, and duration
